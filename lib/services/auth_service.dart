...
class AuthService extends GetxService {
  ...
  String get errorMessage => _errorMessage.value.isEmpty ? 'An unknown error occurred' : _errorMessage.value;
  ...
  Future<bool> login({required String username, required String password}) async {
    errorMessage = '';
    try {
      final response = await dio.post('/api/android/user/login', data: {"username": username.trim(), "password": password});
  ...
  Future<bool> logout() async {
    try {
      await dio.post('/api/android/user/logout').timeout(const Duration(seconds: 5));
    } catch (_) {}
...
//     // Try loading previously stored user
//     final token = await storage.read(key: 'access_token');
//     final userJson = await storage.read(key: 'current_user');
//     if (userJson != null) {
//       _currentUser.value = UserModel.fromJson(jsonDecode(userJson));
...
//         if (response.data['success'] == true) {
//           final data = response.data['data'];
//           final user = UserModel.fromJson(data['user']); // backend returns user object
//           setCurrentUser(user);
